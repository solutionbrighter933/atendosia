...
// Ensure we're using the test publishable key
const stripePublishableKey = import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY;
console.log('🔧 Stripe Configuration Check:');
console.log('Publishable Key:', stripePublishableKey ? 'Present ✅' : 'Missing ❌');
console.log('Key starts with pk_live_:', stripePublishableKey?.startsWith('pk_live_') ? 'Yes ✅ (PRODUCTION MODE)' : 'No ❌ (should be live key for production)');
  ...
  const handleCheckout = async (priceId: string, mode: 'payment' | 'subscription', trialDays?: number) => {
    try {
      ...
      if (!stripePublishableKey) {
        throw new Error('Chave pública do Stripe não configurada. Verifique VITE_STRIPE_PUBLISHABLE_KEY no arquivo .env');
      }

      if (!stripePublishableKey.startsWith('pk_live_')) {
        throw new Error('Use uma chave de produção do Stripe (pk_live_...) para modo de produção. Verifique VITE_STRIPE_PUBLISHABLE_KEY no arquivo .env');
      }
      ...
      console.log('🔧 Using Stripe PRODUCTION mode with key:', stripePublishableKey.substring(0, 20) + '...');
      ...
      if (!response.ok) {
        ...
        console.error('❌ Checkout API error:', response.status, response.statusText, errorData);
        
        // Handle specific Stripe errors
        if (errorData.error && errorData.error.includes('live charges')) {
          throw new Error('Erro de configuração do Stripe. Verifique se as chaves de produção estão corretas no arquivo .env');
        }

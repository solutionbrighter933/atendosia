...
import { Bell, Search, ChevronDown, LogOut, User, MessageSquare, Users, X, Instagram, CreditCard, Calendar, CheckCircle, AlertCircle, Info, Trash2, Package } from 'lucide-react';
...
interface SearchResult {
  type: 'conversation' | 'contact' | 'product' | 'message';
  ...
  const searchInConversationsData = async (query: string): Promise<SearchResult[]> => {
    ...
    try {
      ...
      if (produtosData && produtosData.length > 0) {
        console.log('📦 Found products:', produtosData.length);

        produtosData.forEach(produto => {
          const precoFormatado = produto.preco ? `R$ ${produto.preco.toFixed(2)}` : 'Sem preço';
          const estoqueInfo = produto.estoque !== undefined ? `${produto.estoque} em estoque` : '';

          results.push({
            type: 'product',
            title: produto.nome,
            subtitle: `${precoFormatado} • ${estoqueInfo}${produto.descricao ? ` • ${produto.descricao.substring(0, 40)}...` : ''}`,
            icon: Package,
            action: () => {
              console.log('🎯 Opening product edit modal for:', produto.id);
              setActiveSection?.('my-products');
              setTimeout(() => {
                window.dispatchEvent(new CustomEvent('openProductEdit', { detail: { productId: produto.id } }));
              }, 300);
              setSearchQuery('');
              setShowSearchResults(false);
            },
        ...
        conversasWhatsAppData.forEach(conversa => {
          ...
          results.push({
            ...
            action: () => {
              ...
              console.log('🎯 Setting active section to conversations');
              setActiveSection?.('conversations');
              setTimeout(() => {
                console.log('🎯 Dispatching focusConversation event with data:', {
                  platform: 'whatsapp',
                  conversaId: conversa.conversa_id,
                  numero: conversa.numero_contato
                });
                window.dispatchEvent(new CustomEvent('focusConversation', {
                  detail: { platform: 'whatsapp', conversaId: conversa.conversa_id, numero: conversa.numero_contato }
                }));
          ...
          if (!conversasJaAdicionadas.has(mensagem.conversa_id)) {
            ...
            results.push({
              ...
              action: () => {
                ...
                setActiveSection?.('conversations');
                setTimeout(() => {
                  console.log('🎯 Dispatching focusConversation event for message');
                  window.dispatchEvent(new CustomEvent('focusConversation', {
                    detail: {
                      platform: 'whatsapp',
                      conversaId: mensagem.conversa_id,
                      numero: mensagem.numero,
                      messageTimestamp: mensagem.data_hora,
        ...
        Object.entries(senderGroups).forEach(([senderId, messages]) => {
          ...
          results.push({
            ...
            action: () => {
              ...
              setActiveSection?.('conversations');
              setTimeout(() => {
                console.log('🎯 Dispatching focusConversation event for Instagram:', {
                  platform: 'instagram',
                  senderId,
                  timestamp: latestMessage.data_hora
                });
                window.dispatchEvent(new CustomEvent('focusConversation', {
                  detail: { platform: 'instagram', senderId, timestamp: latestMessage.data_hora }
                }));
                console.log('✅ Event dispatched');
              }, 300);
              setSearchQuery('');
      ...
      const resultadosUnicos = results.filter((result, index, self) =>
        index === self.findIndex(r =>
          ...
          ((r.conversa_id === result.conversa_id) ||
           ...
           (r.product_id === result.product_id))
        )
      ).slice(0, 8);
        ...
        <div className="flex items-center space-x-4">
          <div className="relative" ref={searchRef}>
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Buscar produtos, conversas, mensagens..."
                value={searchQuery}
                onChange={handleSearchChange}
                className="pl-10 pr-10 py-2 w-96 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 transition-all"
              />
              {searchQuery && (
                <button
                  onClick={clearSearch}
                ...
                {searchLoading ? (
                        ...
                        >
                          ...
                          <span className={`text-xs px-2 py-1 rounded-full ${
                            ...
                            {result.type === 'product'
                              ? 'Produto'
                              : result.type === 'conversation'
                              ? `Conversa ${result.platform === 'whatsapp' ? 'WhatsApp' : 'Instagram'}`
                              : `Contato ${result.platform === 'whatsapp' ? 'WhatsApp' : 'Instagram'}`
                            }
                          </span>
                        </button>
                      );
                  ...
                  <div className="py-8 text-center">
                    ...
                    <p className="text-gray-400 dark:text-gray-500 text-xs mt-1">
                      Tente buscar por nome de produto, contato, número, username ou conteúdo de mensagem
                    </p>
                  </div>
                ) : null}
              </div>
          ...
          <div className="relative" ref={userMenuRef}>
            ...
            >
              <div className="relative">
                ...
                <div className={`w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white font-semibold ${profile?.avatar_url ? 'hidden' : ''}`}>
                  {getUserInitials()}
                </div>

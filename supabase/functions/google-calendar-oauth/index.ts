...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Deletar TODAS as integrações antigas deste usuário antes de inserir a nova
    console.log('🗑️ Removendo integrações antigas do usuário...');
    ...
    if (deleteError) {
      console.warn('⚠️ Erro ao deletar integrações antigas (pode ser normal se não havia):', deleteError);
    } else {
      console.log('✅ Integrações antigas removidas');
    }

    // Inserir nova integração
    console.log('💾 Inserindo nova integração do Google Calendar...');
    ...
    if (upsertError) {
      ...
      return new Response(JSON.stringify({
        error: 'Failed to save Google Calendar integration',
        details: upsertError
      }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
